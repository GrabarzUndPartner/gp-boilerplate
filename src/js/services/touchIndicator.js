...
(function() {
    $('a.partial[data-partial*="atoms/link"]').on('click', function(e) {
        ...
        if (!target.classList.contains('js-touch-indicator--animate')) {
            ...
            node.style.cssText =
                'transform: translate3d(' + e.offsetX + 'px, ' + e.offsetY + 'px, 0);';
...
function isNewPageUrl(node) {
    ...
    if (
        linkUrl.toString().replace(linkUrl.hash, '') !==
            winUrl.toString().replace(winUrl.hash, '') &&
        node.href
    ) {
        return true;
    }
